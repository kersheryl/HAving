...
if (isset($_GET['intparties'])) {
  $_SESSION['postid'] = $_GET['intparties'];
}
?>
...
<th colspan='2'>Coming down at</th>
<th> Decision </th>
<th> Status </th>
</tr>";
...
while ($row_2 = mysqli_fetch_array($result_2)){
  ...
  echo "<td align = 'center'>" . $row_2['quantity_req'] . "</td>";
  $datetime = $row_2['dt_req'];
...
<?php
      ...
      echo '<script type="text/javascript"> alert("You\'ve already decided. :)") </script>';
		  ...
		  $new_quantity= ($qty- $row_3['quantity_req']);
...
echo "</td>";
if ($row_2['status']=="Declined") {
    echo "<td style='background-color: #F5B7B1' align = 'center'; >" . $row_2['status'] . "</td>";
  }
  if ($row_2['status']=="Pending") {
    echo "<td style='background-color: #FCF3CF' align = 'center';>" . $row_2['status'] . "</td>";
  }
  if ($row_2['status']=="Approved") {
    echo "<td style='background-color: #D5F5E3' align = 'center'>" . $row_2['status'] . "</td>";
  }
echo "</tr>";
}
echo "</table>";
